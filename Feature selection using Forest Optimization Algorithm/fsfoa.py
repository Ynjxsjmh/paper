...
class Forest (object):
    ...
    def _initialize_forest(self):
        ...
        self.best_tree: list
        '''
        init_num = random.randint(1, self.dimension+1)  # Randomly generated tree num
        '''上面有点问题'''

        for i in range(init_num):
            tree = [random.randint(0, 1) for _ in range(self.dimension+1)] # Randomly generated dimension+1 tree variables with value between 0 and 1
    ...
    def _update_best_tree(self):
        self.forest = sorted(self.forest, key=itemgetter(self.dimension+1), reverse=True) # sort the forest according to the fitness from high to low
        if self.forest[0][self.dimension+1] > self.best_tree[self.dimension+1]:
